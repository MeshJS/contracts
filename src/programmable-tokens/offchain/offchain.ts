...
import { deserializeDatum, resolvePlutusScriptAddress, resolveScriptHash } from "@meshsdk/core";
import {
  buildBaseAddress,
  CredentialType,
  deserializeAddress,
...
import { bootstrapProtocol } from "./transactions/protocolParamMint";
...
export class ProgrammableTokenContract extends MeshTxInitiator {
  ...
  protocolParambootstrap = async (
    ...
    const params = await bootstrapProtocol(
  ...
  registerToken = async (
    assetName: string,
    transferlogicScriptHash: string,
    issuanceLogicScript: string,
    recipientAddress?: string,
    thirdPartylogicScriptHash?: string,
    globalStateLogicScriptHash?: string,
  ) => {
    if (!this.params) {
      throw new Error(
    ...
    const substandardTransferAddress = scriptHashToBech32(transferlogicScriptHash ,undefined, this.networkId, false);
    ...
    const registryMintRedeemer = conStr1([
      ...
      byteString(issuanceLogicScript),
    ]);

    const issuanceRedeemer = conStr0([
      conStr1([byteString(issuanceLogicScript)]),
    ]);
    ...
    const txHex = await this.mesh
      ...
      .withdrawal(substandardTransferAddress, "0")
  ...
  mintTokens = async (assetName: string, issuanceLogicScript: string, recipientAddress?: string | null) => {
    if (!this.params) {
      throw new Error(
        "Protocol bootstrap params are required. Call protocolParambootstrap first.",
      );
